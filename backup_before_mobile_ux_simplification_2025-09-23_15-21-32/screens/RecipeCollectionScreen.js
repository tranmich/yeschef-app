...
const RecipeCollectionScreen = ({ navigation }) => {
  ...
  // Fallback default meal plan structure
  ...
  const displayRecipes = useMemo(() => {
    ...
    console.log('?? COMPUTING displayRecipes for category:', selectedCategory);
  ...
  // ?? GRADUAL TEST - Add back Modal component
  return (
    <ImageBackground 
    ...
    >
      ...
      {/* ?? FIXED: Header positioned correctly outside problematic container */}
      {selectedCategory && (
        <View style={{
          position: 'absolute', 
      ...
      <SafeAreaView style={styles.container}>
        ...
        {/* ?? FIXED: Search positioned outside problematic innerContainer */}
        {selectedCategory && (
          <View style={{
        ...
        <TouchableWithoutFeedback onPress={() => setShowBottomSheet(null)}>
          <View style={styles.innerContainer}>
            ...
            {/* ?? PROPER BOTTOM SHEET MODAL - WORKING! */}
            <Modal
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          ...
                          if (recipe) {
                            ...
                            setShowMealPlanModal(true);
                            // 🎯 NEW: Ensure we have a local meal plan ready
                            if (!currentMealPlan || currentMealPlan.length === 0) {
                              console.log('🎯 Creating default meal plan for modal...');
                              setCurrentMealPlan(getDefaultMealPlan());
                            }
                          }
                        }}
