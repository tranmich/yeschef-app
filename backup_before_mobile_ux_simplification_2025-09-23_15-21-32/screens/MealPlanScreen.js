...
function MealPlanScreen({ navigation }) {
  ...
  // 🔄 Check for local meal plan updates from other screens
  const checkForLocalMealPlan = async () => {
    try {
      ...
      if (localData) {
        ...
        // Only update if we don't have a current plan ID (meaning no saved plan is loaded)
        if (!currentPlanId && isLocal) {
          console.log('✅ Loading local meal plan with', mealPlan.length, 'days');
          ...
          setMealPlanTitle('Local Meal Plan');
          
          // Clear the AsyncStorage item since we've loaded it
          await AsyncStorage.removeItem('localMealPlan');
          console.log('🧹 Cleared local meal plan from AsyncStorage');
        }
      }
    } catch (error) {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      // First check for local meal plan updates from other screens
