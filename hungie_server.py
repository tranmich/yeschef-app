...
# 📧 Waitlist Endpoint - Landing Page Email Capture
@app.route('/api/waitlist', methods=['POST'])
def join_waitlist():
    """
    Handle waitlist signup from landing page
    Stores email and metadata for organizing internal testers
    """
    try:
        ...
        source = data.get('source', 'landing_page')  # hero, final-cta, etc.
        platform_preference = data.get('platform', None)  # iOS, Android, both
        ...
        metadata = {
            ...
            'browser': user_agent[:200]  # Truncate long user agents
        }
        ...
        cursor.execute('''
            INSERT INTO waitlist (
                email, source, platform_preference, ip_address, 
                user_agent, status, metadata
            )
            VALUES (%s, %s, %s, %s, %s, %s, %s)
            RETURNING id
        ''', (
            email, source, platform_preference, ip_address,
            user_agent, 'pending', json.dumps(metadata)
        ))
        ...
        logger.info(f"✅ New waitlist signup: {email} (ID: {waitlist_id}, Source: {source})")
        
        # TODO: Send welcome email (optional)
        # TODO: Add to Google Sheets (optional)
...
# 📊 Admin Endpoint - View Waitlist
@app.route('/api/admin/waitlist', methods=['GET'])
def get_waitlist():
    """
    Get all waitlist signups with filtering and sorting
    Query params:
      - status: pending, invited, accepted, declined
      - platform: iOS, Android, both
      - sort: signup_date, email
    ...
    try:
        ...
        sort_by = request.args.get('sort', 'signup_date')
        ...
        if platform:
            query += ' AND platform_preference = %s'
        ...
        cursor.execute('''
            SELECT 
                ...
                COUNT(*) FILTER (WHERE platform_preference = 'Android') as android
            FROM waitlist
        ''')
...
# 📧 Admin Endpoint - Export Waitlist to CSV
@app.route('/api/admin/waitlist/export', methods=['GET'])
def export_waitlist():
    ...
    try:
        ...
        cursor.execute('''
            SELECT email, source, platform_preference, signup_date, status
            FROM waitlist
        ...
        # Header
        writer.writerow(['Email', 'Source', 'Platform', 'Signup Date', 'Status'])
        ...
        for row in waitlist:
            writer.writerow([
                row['email'],
                row['source'],
                row['platform_preference'] or 'Not specified',
                row['signup_date'].strftime('%Y-%m-%d %H:%M:%S'),
                row['status']
            ])
        ...
        return Response(
            output.getvalue(),
            mimetype='text/csv',
            headers={'Content-Disposition': 'attachment; filename=yeschef_waitlist.csv'}
        )
        
    except Exception as e:
        logger.error(f"❌ Export waitlist error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
    ...
    try:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS waitlist (
                ...
                tester_type VARCHAR(50) DEFAULT 'general',
