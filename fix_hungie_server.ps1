# PowerShell script to fix hungie_server.py corruption
# Remove lines 854-993 and insert clean smart-search function

$filePath = "hungie_server.py"
$lines = Get-Content $filePath

# Keep lines 1-853 (good part before corruption)
$beforeCorruption = $lines[0..852]

# Keep lines 994+ (clean recipe-suggestions function onwards)  
$afterCorruption = $lines[993..($lines.Length-1)]

# Clean smart-search function to insert
$cleanSmartSearch = @'
...
# Combine the parts
$fixedContent = $beforeCorruption + $cleanSmartSearch.Split("`n") + $afterCorruption

# Write the fixed file
$fixedContent | Set-Content "${filePath}.fixed"

Write-Host "✅ Fixed hungie_server.py created as hungie_server.py.fixed"
Write-Host "Original lines: $($lines.Length)"
Write-Host "Fixed lines: $($fixedContent.Length)"
Write-Host "Removed corrupted lines: 854-993"
